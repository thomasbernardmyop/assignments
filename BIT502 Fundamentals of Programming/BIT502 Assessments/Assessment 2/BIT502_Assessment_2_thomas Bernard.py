...
def get_selected_extras():
    extras = []
    if extra1.get():
        extras.append("Book Rental")
    if extra2.get():
        extras.append("Private Area Access")
    if extra3.get():
        extras.append("Monthly Booklet")
    if extra4.get():
        extras.append("Online ebook Rental")
...
def submit():
    ...
    plan_name = membership_plan.get()
    plan_cost = membership_cost
    membership_line = f"{plan_name} (${plan_cost:.2f})"

    member_text = (
        f"First Name: {first}\n"
        f"Last Name: {last}\n"
        f"Address: {entry_address.get()}\n"
        f"Mobile: {mobile}\n"
        f"Membership plan: {membership_line}\n"
        f"Extras: {get_selected_extras()}\n"
        f"Payment plan: {payment_plan.get()}\n"
        f"Library card: {'Yes' if has_library_card.get() else 'No'}\n"
        f"Library card ID: {card if has_library_card.get() else '---'}\n\n"
        f"Extra cost: ${extras_cost:.2f}\n"
        f"Total discount: ${discount:.2f}\n"
        f"Total cost: ${total_cost:.2f}\n"
        f"Weekly cost: ${weekly_cost:.2f}\n"
        f"Payment due: ${total_cost:.2f}\n"
        "---------------------------------------------\n\n"
    )

    script_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(script_dir, "members_data.txt")

    with open(file_path, "a") as file:
        file.write(member_text)
